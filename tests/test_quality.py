...
class TestATBHQFilter:
    ...
    def test_hq_flag_filters_non_hq(self, sample_species_calls_df):
        ...
        hq_only = sample_species_calls_df[
            sample_species_calls_df["HQ"] == "T"
        ]
    ...
    def test_hq_combined_with_qualibact(
        ...
        hq_samples = sample_species_calls_df[
            sample_species_calls_df["HQ"] == "T"
        ]
        enterobacter = hq_samples[
            hq_samples["species"] == "Enterobacter hormaechei_A"
        ]
        ...
        result = filter_by_quality(
            enterobacter, sample_checkm2_df, "Enterobacter hormaechei", {}
        )
        # SAMN003 also passes default quality (comp=98, cont=0.8)
        assert len(result) == 1
        assert result.iloc[0]["sample"] == "SAMN003"

    def test_no_filter_keeps_low_quality(
        self, sample_species_calls_df, sample_checkm2_df
    ):
